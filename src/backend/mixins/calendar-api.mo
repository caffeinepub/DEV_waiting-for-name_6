...
mixin (tokenStore : OAuthLib.TokenStore) {
  ...
  /// passes it as config.auth=?#bearer(token) to CalendarLib.createEvent.
  /// Returns #not_connected if the caller has no stored token, #auth_expired
  /// on a 401/expired-token response from Google, #success with the created
  /// event's id/link, or #error with a message from the Google API response.
  public shared ({ caller }) func create_event(
    title : Text,
    startTime : Text,
    endTime : Text,
  ) : async Types.CreateEventResult {
    switch (OAuthLib.getAccessToken(tokenStore, caller)) {
      case null #not_connected;
      case (?token) {
        let input : Types.EventInput = { title; startTime; endTime };
        await CalendarLib.createEvent(input, token);
      };
    };
  };
};
